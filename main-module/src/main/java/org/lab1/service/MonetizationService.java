...
public class MonetizationService {
    ...
    private static final String REQUEST_TIME_MISSING_LOG = "Request time is missing for payout of application ID: ";
    ...
    private static final String PAYMENT_EXPIRED_MSG = "Payment request expired (more than 30 minutes).";
    private static final String CARD_INVALID_LOG = "Card details are invalid for payout of application ID: ";
    ...
    public MonetizationService(MonetizedApplicationRepository monetizedApplicationRepository,
        ...
        this.logger = logger;
        this.payoutSuccessCounter = Counter.builder(MONETIZATION_RESULT_METRIC)
                .tag(STATUS_TAG, SUCCESS_TAG)
                .register(meterRegistry);
        this.payoutFailCounter = Counter.builder(MONETIZATION_RESULT_METRIC)
                .tag(STATUS_TAG, FAIL_TAG)
                .register(meterRegistry);
        this.payoutExpiredCounter = Counter.builder(MONETIZATION_REASON_METRIC)
                .tag(REASON_TAG, EXPIRED_REASON)
                .register(meterRegistry);
        this.invalidCardCounter = Counter.builder(MONETIZATION_REASON_METRIC)
                .tag(REASON_TAG, INVALID_CARD_REASON)
                .register(meterRegistry);
        this.insufficientFundsCounter = Counter.builder(MONETIZATION_REASON_METRIC)
                .tag(REASON_TAG, INSUFFICIENT_FUNDS_REASON)
                .register(meterRegistry);
        this.appNotFoundCounter = Counter.builder(MONETIZATION_REASON_METRIC)
                .tag(REASON_TAG, APP_NOT_FOUND_REASON)
                .register(meterRegistry);
        this.payoutAmountSummary = DistributionSummary.builder(MONETIZATION_AMOUNT_METRIC)
                .baseUnit(USD_UNIT)
                .register(meterRegistry);
    }
    ...
    public PaymentRequest sendForm(int applicationId, double amount) {
        ...
        logger.info(FORM_GENERATED_LOG + applicationId + REQUEST_ID_LOG + paymentRequest.getApplicationId() +
                AMOUNT_LOG + paymentRequest.getAmount());
        return paymentRequest;
    }

    public String makePayout(PaymentRequest paymentRequest) {
        logger.info(PROCESS_PAYOUT_LOG + paymentRequest.getApplicationId() + AMOUNT_LOG + paymentRequest.getAmount());
        ...
        if (timeElapsed.toMinutes() > REQUEST_EXPIRY_MINUTES) {
            ...
            logger.error(PAYMENT_EXPIRED_LOG + paymentRequest.getApplicationId() + " (more than 30 minutes).");
        ...
        if (monetizedApp.getCurrentBalance() < paymentRequest.getAmount()) {
            ...
            logger.error(INSUFFICIENT_FUNDS_LOG + paymentRequest.getApplicationId() +
        ...
        logger.info(PAYOUT_SUCCESSFUL_LOG + paymentRequest.getApplicationId() +
                AMOUNT_LOG + paymentRequest.getAmount() +
                NEW_BALANCE_LOG + monetizedApp.getCurrentBalance());
