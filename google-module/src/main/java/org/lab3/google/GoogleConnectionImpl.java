...
public class GoogleConnectionImpl implements GoogleConnection {
    ...
    public String uploadFile(String fileName, byte[] fileData) throws IOException {
        ...
        return driveService.files()
                .create(fileMetadata, new com.google.api.client.http.ByteArrayContent("application/octet-stream", fileData))
                .execute()
                .getId();
    }
            ...
            Request request = new Request()
                    .setCreateItem(new CreateItemRequest()
                            ...
                            .setLocation(new Location()
                                    .setIndex(0))); // Добавляем вопросы в начало формы
    ...
    public String createRevenueSpreadsheetWithData(String title, List<String> headers, List<List<Object>> data) throws IOException {
        Spreadsheet spreadsheet = new Spreadsheet()
                .setProperties(new SpreadsheetProperties().setTitle(title));

        SheetProperties sheetProperties = new SheetProperties()
                .setTitle("ApplicationsRevenue")
                .setSheetId(0);
    ...
    @Override
    public void addAppSheets(String googleEmail, String spreadsheetTitle, String appName) throws IOException {
        // 1. Ищем таблицу по названию
        String query = String.format("name='%s' and mimeType='application/vnd.google-apps.spreadsheet'", spreadsheetTitle);
        ...
        int nextSheetId = spreadsheet.getSheets().stream()
        ...
        for (Sheet sheet : spreadsheet.getSheets()) {
            ...
            for (String sheetName : sheetNames) {
                if (title.equals(sheetName)) {
                    deleteRequests.add(new com.google.api.services.sheets.v4.model.Request()
        ...
        for (String sheetName : sheetNames) {
            createRequests.add(new com.google.api.services.sheets.v4.model.Request()
        ...
        data.add(new ValueRange()
                ...
                .setValues(List.of(
                        List.of("applicationId", "revenueFromAdds", "revenueFromDownloads",
                                "revenueFromMicrotransactions", "totalRevenue")
                )));
        // Watched Adds sheet
        data.add(new ValueRange()
                .setRange(appName + " Watched Adds!A1")
                .setValues(List.of(
                        List.of("userId", "addId")
                )));
        // Microtransactions sheet
        data.add(new ValueRange()
                .setRange(appName + " Microtransactions!A1")
                .setValues(List.of(
                        List.of("userId", "microtransactionId")
                )));
        // Downloads sheet
        data.add(new ValueRange()
                .setRange(appName + " Downloads!A1")
                .setValues(List.of(
                        List.of("userId", "appId")
                )));
        ...
        for (SheetProperties sheet : response.getReplies().stream()
                .map(reply -> reply.getAddSheet().getProperties())
                .collect(Collectors.toList())) {

            formatRequests.add(new com.google.api.services.sheets.v4.model.Request()
    ...
    private void handleDownloadEvent(String spreadsheetId, String appName, MonetizationEvent event) throws IOException {
        ...
        appendToSheet(spreadsheetId, appName + " Downloads",
    ...
    private void handlePurchaseEvent(String spreadsheetId, String appName, MonetizationEvent event) throws IOException {
        ...
        appendToSheet(spreadsheetId, appName + " Microtransactions",
                List.of(event.getUserId(), event.getItemId()));
    ...
    private void handleAdViewEvent(String spreadsheetId, String appName, MonetizationEvent event) throws IOException {
        ...
        appendToSheet(spreadsheetId, appName + " Watched Adds",
                List.of(event.getUserId(), event.getItemId()));
    ...
    private void updateRevenueColumn(String spreadsheetId, String sheetName, String column, double amount) throws IOException {
        // 1. Определяем столбец для обновления
        String range = switch (column) {
        ...
        ValueRange currentValue = sheetsService.spreadsheets().values()
        ...
        ValueRange newValue = new ValueRange()
                .setValues(List.of(List.of(Double.toString(currentAmount + amount))));
    ...
    private void updateTotalRevenue(String spreadsheetId, String sheetName) throws IOException {
        ...
        ValueRange revenues = sheetsService.spreadsheets().values()
                .get(spreadsheetId, sheetName + "!B2:D2")
                .execute();
        ...
        ValueRange totalValue = new ValueRange()
    ...
    private void updateApplicationsRevenue(String spreadsheetId, String appName, MonetizationEvent.EventType eventType, double amount) throws IOException {
        // 1. Получаем текущие данные из листа приложения
        ValueRange appRevenue = sheetsService.spreadsheets().values()
        ...
        double adsRevenue = revenueData.get(0) == null || revenueData.get(0).toString().trim().isEmpty()
                ? 0.0
                : Double.parseDouble(revenueData.get(0).toString());

        double downloadRevenue = revenueData.get(1) == null || revenueData.get(1).toString().trim().isEmpty()
                ? 0.0
                : Double.parseDouble(revenueData.get(1).toString());

        double purchasesRevenue = revenueData.get(2) == null || revenueData.get(2).toString().trim().isEmpty()
                ? 0.0
                : Double.parseDouble(revenueData.get(2).toString());

        double totalRevenue = revenueData.get(3) == null || revenueData.get(3).toString().trim().isEmpty()
                ? 0.0
                : Double.parseDouble(revenueData.get(3).toString());

        // 2. Находим строку приложения в ApplicationsRevenue и получаем ID
        ValueRange apps = sheetsService.spreadsheets().values()
        ...
        if (apps.getValues() != null) {
            for (int i = 0; i < apps.getValues().size(); i++) {
                if (apps.getValues().get(i).size() >= 2 && apps.getValues().get(i).get(1).toString().equals(appName)) {
        ...
        if (rowNumber > 0 && appId != null) {
            ValueRange updateData = new ValueRange()
    ...
    private void updateSingleTop(String spreadsheetId, List<AppRevenue> allAppsRevenue) throws IOException {
    ...
    public void updateAppsTop() throws IOException {
        FileList spreadsheets = driveService.files().list()
                .setQ("mimeType='application/vnd.google-apps.spreadsheet' and name contains 'Revenue Statistics'")
                .setFields("files(id, name)")
                .execute();
    ...
    private static class AppRevenue {
        ...
        public AppRevenue(String appName, double totalRevenue, String spreadsheetId, String revenueSheetName) {
