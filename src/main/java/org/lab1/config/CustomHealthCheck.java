...
public class CustomHealthCheck implements HealthIndicator {
    ...
    public Health health() {
        ...
        components.put(POSTGRES_COMPONENT, buildComponentDetails(postgresHealth, stopWatch.getLastTaskTimeMillis()));
        if (postgresHealth.getStatus().equals(Health.down().build().getStatus())) {
            isHealthy = false;
        }

        Health rabbitHealth = performRabbitMQHealthCheck(stopWatch);
        components.put(RABBITMQ_COMPONENT, buildComponentDetails(rabbitHealth, stopWatch.getLastTaskTimeMillis()));
